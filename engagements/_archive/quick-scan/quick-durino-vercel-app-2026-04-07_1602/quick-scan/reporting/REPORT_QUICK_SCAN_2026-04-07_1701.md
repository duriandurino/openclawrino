...
- Quick scan profile `webapp` ran against `https://durino.vercel.app` in `safe` mode and treated the target as a Vercel-hosted Next.js application, capturing 5 meaningful candidate observations with highest provisional severity `High`.
...
- Target traits inferred from artifacts: `api-backed`, `ssr-or-hybrid`, `tls-enforced`
