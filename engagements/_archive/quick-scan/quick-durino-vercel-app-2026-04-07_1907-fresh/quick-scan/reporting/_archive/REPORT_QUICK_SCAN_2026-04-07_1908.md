...
- Quick scan profile `webapp` ran against `https://durino.vercel.app` in `safe` mode and treated the target as a Vercel-hosted Next.js application serving as a public portfolio-style surface, capturing 6 meaningful candidate observations with highest provisional severity `Critical`.
...
| Critical | recon | observed | Attack surface clues: A: 216.198.79.67, 64.29.17.67, Header: server: Vercel, Title: Adrian Alejandrino \| Product Solutions Engineer |
...
- Validate: Attack surface clues: A: 216.198.79.67, 64.29.17.67, Header: server: Vercel, Title: Adrian Alejandrino | Product Solutions Engineer
