...
fallback: gog slides (Google Workspace)
---
...
## ⚠️ Fallback: gog slides

If `cellcog` is not installed, use **gog slides** (Google Workspace) instead:
```bash
gog slides create-from-markdown "Presentation Title" --content-file /path/to/report.md
```
This creates native Google Slides that can be edited and exported as PDF. See gog skill for full documentation.
