...
usage() {
  cat <<'EOF'
Usage: rdp_probe.sh --target <host> --engagement <name> [--base-dir <path>]
EOF
}
...
paths = artifact_paths(root, ${ENGAGEMENT@Q}, "enum", "rdp-probe", ${TARGET@Q})
print(paths["raw"])
print(paths["parsed"])
print(paths["summary"])
PY
...
nmap -Pn -p 3389 -sV "$TARGET" -oN "$RAW_PATH" >/dev/null 2>&1 || true
python3 - <<PY
import json
from pathlib import Path
text = Path(${RAW_PATH@Q}).read_text(encoding='utf-8', errors='ignore')
open_found = '3389/tcp open' in text
payload = {
  'target': ${TARGET@Q},
  'phase': 'enum',
  'script': 'scripts/enum/rdp/rdp_probe.sh',
  'status': 'success',
  'findings': [{'type': 'rdp', 'value': '3389/tcp open'}] if open_found else [],
...
summary = [f"# RDP Probe — ${TARGET}", '', f"- Engagement: ${ENGAGEMENT}", f"- Open: {open_found}"]
