...
from scripts.orchestration.run_enum_profile import load_manifest, substitute


def main() -> int:
    parser = argparse.ArgumentParser(description="Run post-exploit profile manifest")
    ...
    parser.add_argument("--input", default="", help="Input artifact path")
    ...
    if manifest.get("phase") != "post-exploit":
        print(f"manifest phase is not post-exploit: {manifest.get('phase')}", file=sys.stderr)
        return 2

    mapping = {"target": args.target, "engagement": args.engagement, "input": args.input}
    print(f"[*] Running post-exploit profile: {manifest.get('name', manifest_path.stem)}")
    ...
    print("[✓] Post-exploit profile complete")
    return 0
