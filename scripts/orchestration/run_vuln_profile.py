...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run vuln profile manifest")
    ...
    if manifest.get("phase") != "vuln":
        print(f"manifest phase is not vuln: {manifest.get('phase')}", file=sys.stderr)
        return 2

    mapping = {"target": args.target, "engagement": args.engagement, "input": args.input}
    print(f"[*] Running vuln profile: {manifest.get('name', manifest_path.stem)}")
